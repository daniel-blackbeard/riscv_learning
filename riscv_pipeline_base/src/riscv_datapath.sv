...
always_comb begin
 case(ex_if_ctrl.pc_src)
   PCSRC_PC_4  : pc_next = pc_plus_4;               // normal
    PCSRC_PC_IMM: pc_next = pc_plus_imm;             // JAL
    PCSRC_ALU   : pc_next = {alu_res[31:1], 1'b0};   // JALR
    PCSRC_BRANCH: pc_next = alu_branch ? pc_plus_imm : pc_plus_4;  // conditional branches
   default     : pc_next = pc_plus_4;
 endcase
end

assign flush = (ex_if_ctrl.pc_src == PCSRC_PC_IMM) ||                    // JAL — always taken
               (ex_if_ctrl.pc_src == PCSRC_ALU)    ||                    // JALR — always taken
               (ex_if_ctrl.pc_src == PCSRC_BRANCH && alu_branch);        // branch — only if actually taken
...
register_file u_reg(
    ...
    .wr_reg(reg_w_data),
    .addr_rs1(id_inst[19:15]),
    .addr_rs2(id_inst[24:20]),
    .addr_rd(wb_rd_addr),
    .flush(flush),
    .rs1(rs1),
    .rs2(rs2)
...
load_use_detector u_lud(
    .next_rs1(id_inst[19:15]),
    .next_rs2(id_inst[24:20]),
    .curr_rd(ex_rd_addr),
...
reg_id_ex u_idex(
    ...
    .i_rs2(rs2),
    .i_imm(imm),
    .i_rd_addr(id_inst[11:7]),
    .i_rs1_addr(id_inst[19:15]),
    .i_rs2_addr(id_inst[24:20]),
...
always_comb begin
    case(forward_rs2)
        NO_FORWARD  : fw_rs2 = ex_rs2;
        MEM_FORWARD : fw_rs2 = mem_result;
        WB_FORWARD  : fw_rs2 = reg_w_data;
        default     : fw_rs2 = ex_rs2;
    endcase
end

always_comb begin
    case(ex_ex_ctrl.alu_op2)
        OP2_REG: alu_op2 = fw_rs2;
        OP2_IMM: alu_op2 = ex_imm;
    endcase
end
...
always_comb begin
  case(ex_wb_ctrl.reg_src)
    REGSRC_ALU    : ex_result = alu_res;
...
data_memory u_data_mem(
    .clk(clk),
    .rstb(rstb),
    .w_en(mem_mem_ctrl.mem_w_en),
    .mem_signext(mem_mem_ctrl.mem_signed),
    .d_width(mem_mem_ctrl.mem_size),
    .addr(mem_alu_res[DATA_MEM_ADDR_SIZE-1:0]),
    .data_in(mem_rs2),
    .data_out(data_out)
);
...
always_comb begin
  case(wb_wb_ctrl.reg_src)
    ...
    default: reg_w_data = wb_mem_result;
  endcase
end
